...
# ── 3. Re-sign with Info.plist bound ─────────────────────────────────────────
# Swift's linker-signing leaves Info.plist NOT bound to the signature.
# macOS Sequoia/Tahoe rejects IME (e.g. Vietnamese) input for such bundles.
# Force ad-hoc sign with an explicit identifier so Info.plist is sealed in.

echo ""
echo "Code-signing (ad-hoc, Info.plist bound)..."
codesign --force --deep --sign - \
    --identifier "com.fileexplorer.app" \
    --options runtime \
    "$APP"

# Verify the Info.plist is now bound
if codesign -dv "$APP" 2>&1 | grep -q "Info.plist=not bound"; then
    echo "  WARNING: Info.plist still not bound — IME may not work" >&2
else
    echo "  Signed OK (Info.plist bound)"
fi
