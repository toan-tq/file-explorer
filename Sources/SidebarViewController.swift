...
class SidebarViewController: NSViewController, NSOutlineViewDataSource, NSOutlineViewDelegate {
    ...
    // Set while selectItem(for:) syncs the highlight. NSOutlineView posts
    // selection-did-change for programmatic selection too, which would otherwise
    // look like a user click and re-enter navigateTo.
    ...
    func selectItem(for url: URL) {
        ...
        // Compare resolved paths, not URLs. Sidebar URLs come from URL.appending(path:)
        // and carry no trailing slash, while every other source of the current directory
        // (contentsOfDirectory, URL(fileURLWithPath:), NSPathControl) does — so == never
        // matched for most favorites.
        let target = url.standardizedFileURL.path
        for section in sections {
            for item in section.items where item.url.standardizedFileURL.path == target {
